...
export class ErrorBoundary extends Component {
  ...
  render() {
    ...
    return (
      <div className="min-h-screen grid place-items-center bg-orbit-sand p-6" data-testid="app-error">
        <div className="w-full max-w-lg rounded-[22px] border border-black/5 bg-white p-8 shadow-sm">
          <h1 className="font-display text-2xl font-semibold tracking-tight">
            Something broke on this screen
          </h1>
          <p className="mt-3 text-sm leading-relaxed text-orbit-text/65">
            This is a fault on our side, not something you did. Your data is safe — nothing was
            saved or changed.
          </p>

          <p className="mt-4 rounded-xl bg-orbit-sand px-4 py-3 font-mono text-[13px] text-orbit-text/75">
            {String(error?.message || error)}
          </p>

          {showDetail && info?.componentStack && (
            <details className="mt-3">
              <summary className="cursor-pointer text-xs text-orbit-text/55">
                Component stack
              </summary>
              <pre className="mt-2 max-h-56 overflow-auto rounded-lg bg-orbit-text p-3 text-[11px] leading-relaxed text-white/90">
          ...
          <div className="mt-6 flex flex-wrap gap-2">
            <button
              ...
              className="h-10 rounded-full bg-orbit-text px-5 text-sm font-medium text-white transition-colors hover:bg-orbit-text/90"
            ...
            <button
              ...
              className="h-10 rounded-full border border-black/10 px-5 text-sm text-orbit-text/75 transition-colors hover:bg-orbit-sand"
            >
              Reload page
            </button>
            <a
              href="/dashboard"
              className="grid h-10 place-items-center rounded-full px-5 text-sm text-orbit-text/55 transition-colors hover:text-orbit-text"
            >
              Back to dashboard
            </a>
          </div>
